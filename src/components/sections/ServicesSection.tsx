    ...
    >
      <div ref={ref} className="container mx-auto px-4 py-24 md:py-20">
        <div className="grid md:grid-cols-2 gap-12 lg:gap-20 items-center max-w-6xl mx-auto">
          ...
          {/* Derecha: cards apiladas, entran desde la derecha */}
          <div
            className={`space-y-4 transition-all duration-700 ease-out delay-150 ${
              inView ? "opacity-100 translate-x-0" : "opacity-0 translate-x-10"
            }`}
          >
            {services.map((service) => {
              ...
              return (
                <article
                  key={service.number}
                  className="group relative overflow-hidden rounded-r-xl border-l-[3px] border-[#0179B1] bg-[#f8fafc] p-6 transition-colors duration-300 hover:border-[#47DAD6] hover:bg-[#f0f9ff]"
                >
                  <span
                    className="absolute top-2 right-4 text-[4rem] font-black leading-none text-[#0179B1]/[0.08] select-none"
                    aria-hidden="true"
                  >
                    {service.number}
                  </span>
                  <div className="relative flex items-start gap-4">
                    <div className="p-2.5 rounded-lg bg-[#0179B1]/10 text-[#0179B1] shrink-0">

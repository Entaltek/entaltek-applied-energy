...
const InsightLibrary = () => {
  ...
  return <main className="route-page min-h-screen bg-[#F5F9FC] text-[#013762]">
    ...
    <section className="mx-auto max-w-7xl px-4 pb-24 pt-6 sm:px-6 lg:px-8"><div className="flex flex-col gap-5 border-b border-[#013762]/20 pb-5 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-2xl font-bold">{activeCategory ? categoryHeading[activeCategory] : "Publicaciones recientes"}</h2><p className="mt-1 text-sm text-[#365872]">{all.length} {all.length === 1 ? "recurso disponible" : "recursos disponibles"}</p></div><label className="flex w-full items-center gap-2 rounded-lg border border-[#013762]/20 bg-white px-3 py-2 text-[#004C7A] focus-within:outline-2 focus-within:outline-[#0179B1] sm:w-80"><Search className="h-4 w-4" aria-hidden="true" /><span className="sr-only">Buscar artículos</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por tema" className="w-full bg-transparent text-sm text-[#013762] outline-none placeholder:text-[#58718A]" /></label></div>
      {activeCategory && <nav aria-label={`Subsecciones de ${activeCategory.toLowerCase()}`} className="flex flex-wrap gap-2 border-b border-[#013762]/10 py-5">{collections[activeCategory].map((collection) => <a key={collection.id} href={`#${collection.id}`} className="rounded-full border border-[#013762]/15 bg-white px-4 py-2 text-sm font-semibold text-[#004C7A] transition-colors hover:border-[#0179B1] hover:bg-[#E7F3F9] focus-visible:outline-2 focus-visible:outline-[#0179B1]">{collection.title}</a>)}{activeCategory === "Casos" && <a href="#mas-casos" className="rounded-full border border-[#013762]/15 bg-white px-4 py-2 text-sm font-semibold text-[#004C7A] hover:border-[#0179B1] hover:bg-[#E7F3F9]">Operaciones y archivos · en preparación</a>}</nav>}
      {articles.length ? activeCategory ? <div className="space-y-14 pt-10">{collections[activeCategory].map((collection) => { const order: readonly string[] = collection.slugs; const items = articles.filter((item) => order.includes(item.slug)).sort((a, b) => order.indexOf(a.slug) - order.indexOf(b.slug)); return items.length ? <section key={collection.id} id={collection.id} className="scroll-mt-28"><div className="max-w-2xl"><p className="text-xs font-bold uppercase tracking-[0.17em] text-[#0179B1]">{activeCategory}</p><h3 className="mt-2 text-3xl font-bold tracking-tight">{collection.title}</h3><p className="mt-2 leading-relaxed text-[#365872]">{collection.description}</p></div><div className="mt-5 border-t border-[#013762]/20">{items.map((insight) => <ArticleRow key={insight.slug} insight={insight} />)}</div></section> : null; })}{activeCategory === "Casos" && !query && <section id="mas-casos" className="scroll-mt-28 rounded-2xl border border-[#0179B1]/20 bg-[#E7F3F9] p-6 sm:p-8"><p className="text-xs font-bold uppercase tracking-[0.17em] text-[#0179B1]">Próximos casos</p><h3 className="mt-2 text-2xl font-bold">Operaciones y archivos</h3><p className="mt-3 max-w-2xl leading-relaxed text-[#365872]">Esta subsección se abrirá cuando podamos documentar una experiencia real con sus materiales, prueba y permiso de publicación. Mientras tanto, los ejercicios hipotéticos están identificados como tales en Guías.</p></section>}</div> : <div>{articles.map((insight) => <ArticleRow key={insight.slug} insight={insight} />)}</div> : <p className="py-10 text-[#365872]">No encontramos recursos con ese término. Prueba con otra palabra.</p>}</section>
  </main>;
};

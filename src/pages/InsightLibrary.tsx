        ...
        {insightCategories.map((category) => {
          ...
          return <section key={category} id={category.toLowerCase()} className="scroll-mt-10">
            ...
            {categoryInsights.length ? <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{categoryInsights.map((insight) => {
              ...
              return <TransitionLink key={insight.slug} to={`/soluciones/${insight.slug}`} className="group flex min-h-72 flex-col justify-between rounded-2xl border border-[#013762]/10 bg-white p-7 transition-all hover:-translate-y-1 hover:border-[#0179B1]/40 hover:shadow-[0_20px_44px_rgba(1,55,98,0.12)]">
